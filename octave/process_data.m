...
cd ../stats
data_dbscan = load_data(1,10,strcat('stats_dbscan_fusion',experiment));
a = get_stats(data_dbscan, dt);
plot(a(:,7), a(:,1), "linewidth", lw);
data_lidar = load_data(1,10,strcat('stats_lidar',experiment));
b = get_stats(data_lidar, dt);
hold on;
plot(b(:,7), b(:,1),'g', "linewidth", lw);
data_fusion = load_data(1,10,strcat('stats_fusion',experiment));
c = get_stats(data_fusion, dt);
plot(c(:,7), c(:,1),'r', "linewidth", lw);
data_radar = load_data(1,10,strcat('stats_radar',experiment));
